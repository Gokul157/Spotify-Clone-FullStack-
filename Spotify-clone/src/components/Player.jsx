...
const Player = () => {
    const {seekSong,track,seekBar,seekBg,playStatus,play,pause,time,previous,next,volume,adjustVolume} = useContext(PlayerContext);
    ...
    return track?(  
        <div className='h-[10%] bg-black flex justify-between items-center text-white px-4'>
            ...
            <div className='flex flex-col items-center gap-1 m-auto'>
                ...
                <p>{String(time?.currentTime?.minute || 0).padStart(2, '0')}:
                {String(time?.currentTime?.second || 0).padStart(2, '0')}</p>
                    <div ref={seekBg} onClick={seekSong} className='w-[60vw] max-w-[500px] bg-gray-300 rounded-full cursor-pointer'>
                        <hr ref={seekBar} className='h-1 border-none w-0 bg-green-400 rounded-full cursor-pointer'/>
                    </div>
                    <p>{String(time?.totalTime?.minute || 0).padStart(2, '0')}:
                    {String(time?.totalTime?.second || 0).padStart(2, '0')}</p>
                </div>
            </div>
